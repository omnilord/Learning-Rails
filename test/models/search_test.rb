...
class SearchTest < ActiveSupport::TestCase
  ...
  test 'search for articles by title' do
    s1 = Search.where('term @@ to_tsquery(?)', 'foo')
    assert_equal 0, s1.length, 'should return none for missing'

    s2 = Search.where('term @@ to_tsquery(?)', 'Atari')
    assert_equal 1, s2.length, 'should return 1'

    s3 = Search.where('term @@ to_tsquery(?)', 'Article')
    assert_equal 3, s3.length, 'should return 3'
  end

  test 'search for articles by body' do
    s1 = Search.where('term @@ to_tsquery(?)', 'bar')
    assert_equal 0, s1.length, 'Returns none for missing'

    s2 = Search.where('term @@ to_tsquery(?)', 'ass')
    assert_equal 1, s2.length, 'should return 1'

    s3 = Search.where('term @@ to_tsquery(?)', 'body')
    assert_equal 2, s3.length, 'should return 2'
  end

  test 'search for articles by tags' do
    s1 = Search.where('term @@ to_tsquery(?)', 'm')
    assert_equal 0, s1.length, 'Returns none for missing'

    s2 = Search.where('term @@ to_tsquery(?)', 'e')
    assert_equal 1, s2.length, 'should return 1'

    s3 = Search.where('term @@ to_tsquery(?)', 'f')
    assert_equal 2, s3.length, 'should return 2'

    s4 = Search.where('term @@ to_tsquery(?)', 'b')
    assert_equal 3, s4.length, 'should return 3'
  end

  test 'load source from search' do
    s1 = Search.where('term @@ to_tsquery(?)', 'body')
    ...
    s2 = Search.where('term @@ to_tsquery(?)', 'f')
